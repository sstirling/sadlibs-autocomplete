...
function create_cards_house(data,town_val) {
 ...
 $('.house').append(
    $("<div class='legislators'><p>If you die under suspicious or strange circumstances, the <span class='robotext'>" + key.office + " Medical Examiner's Office</span> will be tasked with investigating your case.<br><p>In <span class='robotext'>" + key.year + "</span>, the most recent data available, this office completed <span class='robotext'>" + key.u60per + "% </span>of its cases in under 60 days, which <span class='robotext'>" + key.accred + " </span>make it eligible for accreditation from the National Association of Medical Examiners.<br><img src='accred-img/" + key.chart +"' width='100%'></div>")
  );
  $('.house').append(
    $("<div class='legislators'><p>In 2016, the <span class='robotext'>" + key.office + " </span>office tossed out <span class='robotext'>" + key.per_released + "%</span> of its cases without ever viewing the body, which is <span class='robotext'>" +key.release_state_ml+ " </span>than the state average of <span class='robotext'>" +key.nj_per_released + "% </span>.<br><p>This office completed <span class='robotext'>" + key.autopsy + "</span> autopsies, about <span class='robotext'>" + key.per_autopsy + "%</span> of the cases they handled. This is <span class='robotext'>" + key.auto_days_hl + " </span>than the state average of <span class='robotext'>" + key.nj_per_auto + "%</span>.<br><p>In <span class='robotext'>" + key.year + "</span> it took this office <span class='robotext'>" + key.days_overall + "</span> days to complete an average case, which is <span class='robotext'>" + key.days_hl + "</span> than the state average of <span class='robotext'>" + key.days_state + "</span>. <img src='chart-img/" + key.chart +"' width='100%'></div>")
  );
